	...
	void __user *arg)
{
	...
	/* HTC_END */
	info.dual_camera = sdata->dual_camera; /* HTC sungfeng */
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_ACTUATOR_INFO: {
		...
		cdata.is_cal_supported = 0; // HTC pg 20130225 vcm cal
		...
		if (p_mctl->actctrl->a_config) {
			...
			cdata.is_cal_supported = p_mctl->actctrl->is_cal_supported; // HTC pg 20130225 vcm cal
	...
	case MSM_CAM_IOCTL_ACTUATOR_IO_CFG: {
		...
		if (p_mctl->actctrl->a_config) {
			...
			act_data.is_cal_supported = 0; // HTC pg 20130225 vcm cal
...
//CC140211
		...
		s_ctrl->first_init=0;
		rc = s_ctrl->func_tbl->sensor_mode_init (s_ctrl, MSM_SENSOR_REG_INIT, 0);//CC140207
		if (rc < 0) {
...
#if 0 //HTC sungfeng no need to stop stream before power off
