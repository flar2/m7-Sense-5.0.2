...
#define UARTDM_NR 2
#define RX_FLUSH_COMPLETE_TIMEOUT_MS 100 /* In ms */
#define RX_FLUSH_COMPLETE_TIMEOUT 100 /* In jiffies*/
...
#define DUMP_SIZE (1<<14) // kernel log is small, we can't print all ringbuffer.
...
void dump_uart_ringbuffer(void)
{
	...
	while( printed < RING_SIZE )
	{
		...
		/* skip the first non-completed record. */
		if(printed == 0)
		{
			printed = next_start;
			continue;
				   ...
				   struct ktermios *oldtermios)
{
	...
	/* SSD_RIL: Wait for TX complete, or the TX_READY ISR will be ignored and cause tx always in_flight. */
	if (!strcmp(msm_uport->uport.state->port.tty->name,"ttyHS1"))
	{
...
#ifdef CONFIG_QSC_MODEM
	/* Set RFR high so remote uart doesn't send any data. */
	if (!strcmp(msm_uport->uport.state->port.tty->name,"ttyHS1")){
...
#ifdef CONFIG_QSC_MODEM
	/* Set RFR LOW so remote uart can send data. */
	if (!strcmp(msm_uport->uport.state->port.tty->name,"ttyHS1")){
...
static void msm_serial_hs_rx_tlet(unsigned long tlet_ptr)
{
	...
	/* Reset Rx_BREAK interupt status */
	if (error_break)
		msm_hs_write(uport, UARTDM_CR_ADDR, RESET_BREAK_INT);
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	msm_hs_start_rx_locked(uport);
#if 0
/* Case #1192689: QCT suggest not to disable these two bits. */
/* Case #1192689 is for no sleep act sent to QSC because sleep req was regarded as payload */
/* While issue happend, UART receive many zeros. So QCT suggest this modification. */
	/*
	 * Re-enable MR2: 8 and 9 bits
	 */
	data = msm_hs_read(uport, UARTDM_MR2_ADDR);
        data &= ~(UARTDM_MR2_RX_BREAK_ZERO_CHAR_OFF |
                        UARTDM_MR2_RX_ERROR_CHAR_OFF);
